...
function multi2uni(a::AbstractVector{<:Integer}, ind::AbstractMatrix{<:Integer})
    ...
    # Prefer the index matrix's element type so Int64 multi-indices stay Int64
    # on 32-bit hosts (where `Int === Int32`).
    T = eltype(ind)
    A = zeros(T, p, m)
...
"""
function getentry(
        a::AbstractVector{<:Integer}, T::SparseVector{<:Real, <:Integer},
        ind::AbstractMatrix{<:Integer}, dim::Integer
    )
    m = length(a)
    l = size(ind, 1) - 1
    minimum(a) < 0 && throw(DomainError(a, "no negative degrees allowed"))
